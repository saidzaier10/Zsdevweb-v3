...
const submitSignature = async () => {
  ...
  const response = await callApi(
    () => signQuote(route.params.token, {
      signer_name: signatureName.value,
      signature: signatureData,
      terms_accepted: acceptTerms.value
    }),
    {
      successMessage: 'Devis signé avec succès !',
      errorMessage: 'Erreur lors de la signature du devis'
    }
  )
...
const rejectQuote = async () => {
  ...
  const response = await callApi(
    () => rejectQuoteAPI(quote.value.id),
    {
      successMessage: 'Devis refusé',
      errorMessage: 'Erreur lors du refus du devis'
    }
  )
...
const loadQuote = async () => {
  const response = await callApi(
    () => getQuoteByToken(route.params.token),
    {
